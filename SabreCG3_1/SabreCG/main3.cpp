//* 20161229 for test findOneNewColumn consumption
...
//	schedule->print();
	...
	*/
	...
	//*测试findNewOneColumn使用内存的情况

	//*设置每个lof的dual, 使得findNewOneColumn解出的reduced cost肯定是负的
	for (int i = 0; i < legList.size(); i++)
	{
		if (legList[i]->isMaint())
		{
			legList[i]->setDual(Util::w_cancelMtc);
		}
		else
		{
			legList[i]->setDual(Util::w_cancelFlt);
		}
	}

	vector<Lof* > lofVector;
	Lof* tempLof;

	int counter = 0;
	int iteration = 0;

	cout << "######## Memory Test Begin ########" << endl;

	while(1)
	{
		cout << "######## Subproblem Iteration " << iteration << " Begin ########" << endl;
		for(int i = 0; i < aircraftList.size(); i++)
		{
			
			tempLof = model->findNewOneColumn(aircraftList[i]);
			//tempLof->print();
			lofVector.push_back(tempLof);
			counter++;
		}
		cout << "******** counter is " << lofVector.size() << "********" << endl;
		cout << "######## Subproblem Iteration " << iteration << " End ########" << endl;
		iteration++;
	...
	cout << "******** lofVector size is " << lofVector.size() << "********" << endl;
	cout << "print last lof" << endl;
	lofVector[lofVector.size()-1]->print();
	cout << "######## Memory Test End ########" << endl;
