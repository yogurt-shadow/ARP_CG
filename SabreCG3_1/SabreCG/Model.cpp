...
vector<Lof *> Model::findNewMultiColumns(Aircraft* aircraft)
{
	...
	{ //* check each node in topological order, to do relax operation
		...
		for (int j = 0; j < thisLeg->getNextLegList().size(); j++)
		{
			...
			{
				edgeProcessFltFlt(thisLeg, nextLeg, aircraft);//##寻找路径时使用的edge cost 包括delay,swap,flight dual
			}
...
void Model::edgeProcessMaintFlt(SubNode* subNode, Leg* nextLeg, Aircraft* aircraft)
{
	...
	if (thisLeg->getAircraft() == aircraft) { // 如果thisLeg的maitenance与aircraft匹配
		...
		if (!thisLeg->getSubNodeList().empty()) { // 如果thisLeg的maitenance与aircraft不匹配，但subNodeList非空
...
void Model::edgeProcessMaintMaint(SubNode* subNode, Leg* nextLeg, Aircraft* aircraft)
{
	...
	if (nextLeg->isMaint()) { // 如果nextLeg也是maitenance
		if (nextLeg->getAircraft() == aircraft) { // 如果aircraft与maintenance匹配
			...
			if (!thisLeg->getSubNodeList().empty()) { // thisLeg是maint, aircraft不匹配，node cost不可能不被修改过
