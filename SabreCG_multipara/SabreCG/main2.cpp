...
{ 
	...
	for(int i = 0; i < aircrafts.size(); i++)
	{
		...
		for(int j = 0; j < stationList.size(); j++)  /// 遍历stationList中保存的station pointer, 如果depStation还没有加入stationList, 加入之; 得到depStation, arrStation的指针
		{
	...
	for (int i = 0; i < aircraftList.size(); i++) /// 设置机场的departure aircraft信息
	{
	...
	for (int i = 0; i < stationList.size(); i++)
	{
		stationList[i]->setLegNum(legList.size()); // 既然所有station的legNum都是一样的，干嘛不把legNum设为static?
	}
	...
	for(int i = 0; i < airportClosures.size(); i++)
	{
		...
		pair<time_t, time_t> closeTime;  /// 建一个pair 保存closeTime, 起始和结束时间
	...
	//std::cout << "unit:" << NEWAOUNT << std::endl;
	std::cout << "unit:" << Util::newamount << std::endl;
	std::cout << "parallel:" << Util::threadSize << std::endl;
	...
	schedule->setOperLofList(); /// added 然而并没有什么用
...
//	schedule->print();
	...
	model->populateLofByAircraft();  /// agg model用col enumeration求解模型，并把选出来的lof复制给aircraft，每架aircraft有一个独立的lofList
	model->delayLofsCrossDay();  /// 每架aircraft的lofList做cross day delay, 每架飞机的lofList和在一起放在getOperLofList, 给后面modelIP用
	model->computeLofCost(); /// 每架aircraft的lofList的lof 计算cost
	...
	*/
	...
	// 打印数据到表格
	ofstream outFile;
	outFile.open("E:\\Sabre\\SabreCG4_multipara\\data_port.txt", ios::out);
	if(!outFile.is_open())
	{
		cout << "fail to open data_port" << endl;
		exit(0);
	}

	// 打印表头
	outFile << input_path << endl;
	outFile << "threadSize" << "\t"
		<< "newamount" << "\t"
		<< "LP Obj Val" << "\t"
		<< "IP Obj Val" << "\t"
	...
	// 测试lofs per subproblem的影响
	int threadSizes[8] = {1,2,3,4,5,6,7,8};
	// 测试threadsize的影响
	int newAmounts[7] = {1,2,3,4,5};

	for (int i = 0; i < 8; i++)
	{
		Util::threadSize = threadSizes[i];
		cout << "####################### THREADSIZE " << Util::threadSize << "#######################" << endl;
		for (int j = 0; j < 5; j++)
		{
			Util::newamount = newAmounts[j];
			cout << "##################### NEWAMOUNT " << Util::newamount << "#####################" << endl;
			...
			int runtime = (endTime - startTime)/CLK_TCK;
			cout << "total run time is " << runtime << " seconds" << endl;

			outFile << Util::threadSize << "\t\t"
				<< Util::newamount << "\t\t"
				<< model->getLpObjValue() << "\t\t"
				<< model->getIpObjValue() << "\t\t"
				<< model->getLpTime()/CLK_TCK << "\t\t"
				<< model->getSpTime()/CLK_TCK << "\t\t"
				<< model->getIpTime()/CLK_TCK << "\t\t"
				<< runtime << "\t\t"
				<< model->getCount() << "\t\t"
				<< model->getLofSize()
				<< endl;
			...
			//cout << "number of lofs generated per subproblem " << Util::newamount << endl;
			cout << "##################### NEWAMOUNT " << Util::newamount << " END #####################" << endl;
		}
		cout << "####################### THREADSIZE " << Util::threadSize << " END #######################" << endl;
	}
